...
bool rnw_content(TSLexer *lexer) {
  ...
  while (((lexer->get_column(lexer) != 0) || (lexer->lookahead != '@')) &&
         (!lexer->eof(lexer))) {
...
bool rnw_sig_end(TSLexer *lexer) {
  ...
  if (res) {
    lexer->result_symbol = _RENV_SIG_END;
    lexer->mark_end(lexer);
  }
...
bool word_or_sig(TSLexer *lexer) {
  ...
  if ((col == 0) && (val != '\\')) {
    ...
    if (issig) {
      lexer->mark_end(lexer);
      lexer->result_symbol = _RENV_SIG_BEG;
      return (true);
    }
  }
  ...
  // If we advanced once already, then fval is "<", twice, we wouldn't be here.
  // So if we're not at a '\\' or whitespace, keep marking the line until we hit
  // whitespace.
  if (lexer->lookahead != '\\') {
...
unsigned tree_sitter_rnoweb_external_scanner_serialize(void *payload,
                                                       char *buffer) {
  return 0;
};
void tree_sitter_rnoweb_external_scanner_deserialize(void *payload,
                                                     const char *buffer,
                                                     unsigned length){};

bool tree_sitter_rnoweb_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[_LATEX_WORD] || valid_symbols[RENV_INLINE] ||
      valid_symbols[_RENV_SIG_BEG]) {
    res = word_or_sig(lexer);
  } else if (valid_symbols[_RENV_SIG_END]) {
